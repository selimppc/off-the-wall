...
$(function () {
    $('#tabs').tabs();
    //&offtwl__is_add_cart=<?php echo $_GET['offtwl__is_add_cart']; ?>";
    url = "/?file=getframednew&offtwl__iproduct_id=" + $("#offtwl__catzid").val() + "&icat_id=" + $("#catzid").val() + "&offtwl__is_add_cart=" + $("#offtwl__is_zadd_cart").val();
    $.get(url, function (d) {
        // $("#body").html(d);
        $("a.zoom").fancybox({
    ...
    if ($("#offtwl__glass_available").val() != 0) {
    ...
    $("#add2cart").live("click", function () {
        ...
        if ($("#offtwl__is_add_cart").val() == '1' && $("#offtwl__iframe_id").val() == "") {
    ...
    var $body = $('body').first(),
        color_picker_board = $('.offtwl__matboard-sqr');
    color_picker_board.on('click', function () {
        $body.css({
            background: '#' + $(this).attr('data-color-code')
        })
    });
    ...
    $("#offtwl__iframe_categori_id").val(50);
...
function offtwl__viewfull() {
    var lrgimg = "/frame-preview.html?offtwl__sess=" + $("#offtwl__sess").val() + "&show=1&a=p" + dourl() + "&prod_id=" + $("#offtwl__iproduct_id").val();
    lrgimg += '&out=/' + $("#offtwl__sess").val() + '/prod_images/lrg_print_tmp_' + $("#offtwl__iproduct_id").val() + '.jpg';
...
function selectframelist(e, no) {
    var c = $("#offtwl__iframe_categori_id").val();
    var code;
    if (no == "2") {
        code = $("#fcodeSel").val();
    }
...
function removeframe() {
    ...
    $("#offtwl__selectedframeremove").html('');
    $('#fcodeSel').val('none');
    $("#offtwl__iframe_id").val('');
    $("#glass_type").val('NG');
    $("#backing_type").val('na');
...
function SelectFrameCode(Obj) {
    ...
    /*	var code = $("#code_" + obj_value).val();
     ...
     $("#offtwl__iframe_id").val(obj_value);
     ...
     $("#offtwl__selectedframe").html("<strong>Selected Frame: </strong>"+ code +" <a href='javascript: void(true);' onclick='removeframe();'>(remove)</a>");
     $("#framez, #glassez, #backingz").show();
     $("#glass_type").val('CG');
     if($("#offtwl__glass_available").val() == 0) { $("#glass_type").val('NG'); $("#glassez").hide(); }
     adjustdim();
     //offtwl__selectedframe
     */
...
function CanvasFrameImage(elem) {
    ...
    this.width = $elem.parent().find('.offtwl__framed-image-placeholder').first().outerWidth();
    this.height = $elem.parent().find('.offtwl__framed-image-placeholder').first().outerHeight();
    ...
    this.setSize = function (width, height) {
        this.width = width || $elem.parent().find('.offtwl__framed-image-placeholder').first().outerWidth();
        this.height = height || $elem.parent().find('.offtwl__framed-image-placeholder').first().outerHeight();
        elem.width = this.width;
        elem.height = this.height;
    };
    ...
    this.drawSlipImage = function (frame) {
        ...
        FRAME_IMAGE.onload = function () {
            ...
            // $constructor_this.ctx.drawImage($('#offtwl__framedimg')[0], FRAME_IMAGE.width, FRAME_IMAGE.width, offtwl__framedimg.width - (FRAME_IMAGE.width * 2), offtwl__framedimg.height - (FRAME_IMAGE.width * 2));
            // $constructor_this.ctx.clearRect(0, 0, offtwl__framedimg.width, offtwl__framedimg.height);
    ...
    this.drawImage = function (frame) {
        var offtwl__framedimg = document.getElementById('offtwl__framedimg');
        ...
        // SCALE =  (frame.frameWidth * 8) > 90 ? (frame.frameWidth * 3) : (frame.frameWidth * 4);
        // SCALE = frame.frameWidth * 1.8;
        ...
        FRAME_IMAGE.onload = function () {
            ...
            // $constructor_this.ctx.drawImage($('#offtwl__framedimg')[0], FRAME_IMAGE.width, FRAME_IMAGE.width, offtwl__framedimg.width - (FRAME_IMAGE.width * 2), offtwl__framedimg.height - (FRAME_IMAGE.width * 2));
            // $constructor_this.ctx.clearRect(0, 0, offtwl__framedimg.width, offtwl__framedimg.height);
            $constructor_this.ctx.drawImage($('#offtwl__framedimg')[0], 0, 0, $constructor_this.width, $constructor_this.height );

            $constructor_this.ctx.drawImage(canvas, 0, 0, offtwl__framedimg.width, offtwl__framedimg.height);
            // $constructor_this.ctx.drawImage(canvas, 0, 0, 100, 100 * canvas.height / canvas.width);
            $constructor_this.ctx.restore();
        };
    };
...
var canvasFrame = new CanvasFrameImage('#offtwl__draw_frame');
...
console.log('hi');
var offtwl__item_size = $('#offtwl__item_size'),
    frame = $('.frame');
$.get('./static/json/price-list.json', function (data) {
    var total_prodsize = parseFloat(offtwl__item_size.attr('data-total-size')),
        priceRanges = [];
...
$(".selectframe").live('click', function () {
    ...
    frame_weight.val($(this).parent().parent().parent().find(".fweight").html());
    $('#offtwl__framedimg').addClass('activated');
    ...
    $("#offtwl__iframe_id").val($(this).parent().parent().parent().find(".id").html());
    ...
    $("#offtwl__selectedframeremove").html("<a href='javascript: void(true);' onclick='removeframe();'>(remove)</a>");
    ...
    var glassAvailable = document.getElementById('offtwl__glass_available');
    var selection = document.getElementById('offtwl__iframe_categori_id');
...
$(".selectslip").live('click', function () {
    if ($("#framez").css("display") != "none") {
        ...
        $("#offtwl__islip_id").val($(this).parent().parent().parent().find(".id").html());
...
$(".selectfillet").live('click', function () {
    if ($("#matz1").css("display") != "none") {
        ...
        $("#offtwl__ifillet_id").val($(this).parent().parent().parent().find(".id").html());
...
function offtwl__draw_frame(url, type) {
...
function fixdim() {
    ...
    $("#offtwl__framedimg, #mat1, #mat2, #mat3, #slip, #fillet").removeAttr("style");


    $(".price").html("<img src='./static/images/loading.gif' width='16' height='16' />");

    if (f != 0) {
        var frameurl = "/frame-me.html?offtwl__sess=" + $("#offtwl__sess").val() + "&id=frame&print_width=" + (pw + (sl + mat1 + mat2 + mat3 + fl) * 2) + "&print_height=" + (ph + (sl + mat1 + mat2 + mat3 + fl) * 2) + "&frame_width=" + f + "&frame_code=" + $("#framez > .fimg").html() + "&nc=" + Math.random() * 100001;
        /*$("#frame").css({
        ...
        offtwl__draw_frame(frameurl, "frame");
        $("#largerimg").show();
    }

    if (s != 0) {
        var slipurl = "/frame-me.html?offtwl__sess=" + $("#offtwl__sess").val() + "&id=slip&print_width=" + (pw + (mat1 + mat2 + mat3 + fl) * 2) + "&print_height=" + (ph + (mat1 + mat2 + mat3 + fl) * 2) + "&frame_width=" + sl + "&frame_code=" + $("#slipz > .fimg").html() + "&nc=" + Math.random() * 100001;
        /*$("#slip").css({
            "width": (pw + (sl + mat1 + mat2 + mat3 + fl) * 2) + "px",
        ...
        offtwl__draw_frame(slipurl, "slip");
        top = sl;
    }
    ...
    if (m1 != 0) {
        ...
        $("#offtwl__framedimg").css("border", "1px solid white");
        top = mat1 - 1;
    }
    ...
    if (fi != 0) {
        var filleturl = "/frame-me.html?offtwl__sess=" + $("#offtwl__sess").val() + "&id=fillet&print_width=" + (pw) + "&print_height=" + (ph) + "&frame_width=" + fl + "&frame_code=" + $("#filletz > .fimg").html() + "&nc=" + Math.random() * 100001;
        /*$("#fillet").css({
            "width": (pw + (fl * 2)) + "px",
        ...
        $("#offtwl__framedimg").css({"border": "0"});
        if (f == 0) {
            $("#frame-it-frame-loader").fadeTo("slow", 0);
        }
        offtwl__draw_frame(filleturl, "fillet");
        top = fl;
    }
    ...
    // $("#offtwl__framing-image").animate({"height": $("#frame").height()}, 'slow');
    // $("#offtwl__framedimg").css({"width": (pw) + "px", "height": (ph) + "px", "margin-top": top + "px"});
    // $("#offtwl__framedimg").css({"width": (pw) + "px", "height": (ph) + "px"});
    console.log('offtwl__framedimg height applying');
    doprice();
    return;
}
...
function doprice() {
    ...
    ajaxManager.add({
        ...
        success: function (d) {
            ...
            if ($("#catzid").val() == '1510') {
                fprice = fprice * parseFloat($("#offtwl__csrate").val());
                sprice = sprice * parseFloat($('#offtwl__csrate').val());
            }
...
function adjustdim() {
    ...
    $("#offtwl__prodinfo").html("Finished Product Approx. Size:  " + roundNumber(fw) + " x " + roundNumber(fh) + " cm - Internal Size (" + roundNumber(iw) + " x " + roundNumber(ih) + " cm)");

    $("#offtwl__prodzinfo").show();
    $("#offtwl__prodzerror").hide();

    if (m1 != 0) {
        var merr = check_mat(iw, ih);
        if (merr != "") {
            $("#frame-it-frame-loader").hide();
            $("#offtwl__prodzinfo").hide();
            $("#offtwl__prodzerror").find('.msg').html(merr);
            $("#offtwl__prodzerror").show();
            return;
        }
    }
    ...
    if (fr != 0 && s == 0) {
        ...
        if (err != "") {
            $("#frame-it-frame-loader").hide();
            $("#offtwl__prodzinfo").hide();
            $("#offtwl__prodzerror").find('.msg').html(err);
            $("#offtwl__prodzerror").show();
            return;
        }
    }
...
function add2cart() {
    ...
    var file = "newaddtocart", action = "addtocart", offtwl__iproduct_id = $("#offtwl__iproduct_id").val(), icat_id = $(
        "#icat_id").val();
    var offtwl__iframe_id = $("#offtwl__iframe_id").val(), glass_type = $("#glass_type").val(), totalmat = $("#totalmat").val();
    ...
    /*    var furl = "/frame-preview.html?a=p" + dourl() + "&prod_id=" + $("#offtwl__iproduct_id").val();
        furl += '&out=/' + $("#offtwl__sess").val() + '/prod_images/final_lrg_print_' + $("#offtwl__iproduct_id").val() + '.jpg';*/

    var furl = "/frame-preview.html?a=p" + "&product_type=" + $("#product_type").val() + "&total_price_amount=" + $("#total_price_amount").val() + "&frame_weight=" + $("#frame_weight").val();
    ...
    $.ajax({
        method: 'POST',
        url: './frame-preview.html',
        data: furlData,
        success: function (response) {
            console.log(response);
            var url = '/index.php?file=' + file + '&action=' + action + '&offtwl__iproduct_id=' + offtwl__iproduct_id + '&icat_id=' + icat_id + '&offtwl__iframe_id=' + offtwl__iframe_id;
            ...
            url += '&swid=' + $("#slipz > .size").html() + "&offtwl__islip_id=" + $("#offtwl__islip_id").val() + "&scode=" + scode;
            url += '&fiwid=' + $("#filletz > .size").html() + '&offtwl__ifillet_id=' + $("#offtwl__ifillet_id").val() + "&ficode=" + ficode;
            url += prodstring;
            window.location = url;
        }
    });
...
function offtwl__editme() {

    alert("Sumer");
    $("#offtwl__editholder").show();
    $("#offtwl__editbtn").hide();
    $("#newwidth").val($("#inner_width").val());
    $("#newheight").val($("#inner_height").val());
}

function offtwl__cancelme() {
    $("#offtwl__editholder").hide();
    $("#offtwl__editbtn").show();
    //$("#newwidth").val($("#inner_width").val());
    //$("#newheight").val($("#inner_height").val());
}

function offtwl__submit_new() {
    var w = $("#newwidth").val();
    var h = $("#newheight").val();
...
// var icat_id = $("#icat_id").val();
    ...
    if (w != "" && h != "") {
        ...
        $("#offtwl__item_size").html("Product Size: " + w + " x " + h + " cm");
        $("#offtwl__editholder").hide();
        $("#offtwl__editbtn").show();
        adjustdim();
    }
