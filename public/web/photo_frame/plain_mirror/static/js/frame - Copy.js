...
$(function () {
    ...
    $("#backing_type").live("change", function () {
        update_price($(this).val(), "backingz");
    });
...
function selectframelist(e, no) {
    ...
    console.log($(e.target).find('option:selected').data('url'));
...
function CanvasFrameImage(elem) {
    ...
    /*   this.drawImage = function (img, position) {
     ...
     console.log($constructor_this);
     };
     };*/
    this.drawImage = function (frame) {
        $constructor_this.setSize();
        console.log('drawing');
        console.log($constructor_this.width);
        console.log($constructor_this.height);
        console.log('drawing');
        var FRAME_IMAGE = new Image();
        FRAME_IMAGE.src = frame.src;
        FRAME_IMAGE.onload = function () {
            console.log(FRAME_IMAGE.width);

            var tileHeight = FRAME_IMAGE.height;

            // TODO: FIX THE ISSUE FOR IOS/SAFARI WHEN SCALING
            var ctxScale = parseFloat(((SCALE * FRAME_IMAGE.width) / tileHeight).toFixed(2));

            var ctxScaleWidth = parseInt(elem.width / ctxScale);
            var ctxScaleHeight = parseInt(elem.height / ctxScale);

            $constructor_this.ctx.fillStyle = $constructor_this.ctx.createPattern(FRAME_IMAGE, 'repeat-x');


            ////// TOP //////
            $constructor_this.ctx.save();
            $constructor_this.ctx.scale(ctxScale, ctxScale);
            $constructor_this.ctx.fillRect(0, 0, ctxScaleWidth, FRAME_IMAGE.height);
            $constructor_this.ctx.restore();

            ////// BOTTOM //////
            $constructor_this.ctx.save();
            $constructor_this.ctx.scale(ctxScale, ctxScale);
            $constructor_this.ctx.beginPath();
            $constructor_this.ctx.translate(ctxScaleWidth, ctxScaleHeight);
            $constructor_this.ctx.rotate(180 * PIRAD);
            $constructor_this.ctx.moveTo(0, 0);
            $constructor_this.ctx.fillRect(0, 0, ctxScaleWidth, FRAME_IMAGE.height);
            $constructor_this.ctx.rotate(180 * PIRAD);
            $constructor_this.ctx.restore();
            ...
            $constructor_this.ctx.save();
            $constructor_this.ctx.scale(ctxScale, ctxScale);
            $constructor_this.ctx.translate(0, ctxScaleHeight);
            $constructor_this.ctx.rotate(-90 * PIRAD);
            $constructor_this.ctx.beginPath();
            $constructor_this.ctx.moveTo(0, 0);

            $constructor_this.ctx.lineTo(FRAME_IMAGE.height, FRAME_IMAGE.height);
            $constructor_this.ctx.lineTo(ctxScaleHeight - FRAME_IMAGE.height, FRAME_IMAGE.height);
            $constructor_this.ctx.lineTo(ctxScaleHeight, 0);
            $constructor_this.ctx.lineTo(0, 0);

            $constructor_this.ctx.closePath();
            $constructor_this.ctx.clip();
            $constructor_this.ctx.fill();
            $constructor_this.ctx.rotate(90 * PIRAD);
            $constructor_this.ctx.translate(0, -ctxScaleHeight);
            $constructor_this.ctx.restore();
            ...
            $constructor_this.ctx.save();
            $constructor_this.ctx.scale(ctxScale, ctxScale);
            $constructor_this.ctx.translate(ctxScaleWidth, 0);
            $constructor_this.ctx.rotate(90 * PIRAD);
            $constructor_this.ctx.beginPath();
            $constructor_this.ctx.moveTo(0, 0);
...
canvasFrame.drawImage({
    // width: '',
    src: 'images/frame_images/thumb/th_cross_frame_1227.jpg',
});
